...
The goal here is to automate response, in real-time, of increased guest traffic.  Statistics are kept within a short time window, a couple of hours, for a set of IP buckets.  To keep things fast, these counters are maintained in memory resident tables where possible.  They must be cleaned out periodically for the sake of speed, and to ensure you don't blow up memory...

Logged-in users are not impacted.  Guests & bots are.

WARTT does not allow you to block specific ASNs, countries or IP ranges.  Update your .htaccess for that.  WARTT tracks activity for all potential ASNs, and will take actions on the problem ones.  It doesn't know in advance which one is going to be a problem - it accumulates the stats to determine that.
...
The threshold checks work both ways: when traffic falls back below the threshold, the block is lifted.
...
There are three ways to identify ASN or country:
 ...
 - ASN or country may be looked up by WARTT.  This requires that the SMF Web Access Log Analyzer (WALA) mod is installed, and the DBIP tables for ASN & country are loaded and current.

WARTT & WALA work hand-in-hand.  

Potential actions include:
 - Block - disallow & return an http 429, "too many requests"; a block percentage may be specified, e.g., to thin the herd
...
Guidance:
 ...
 - Start simple.  A rule or two.  Log entries for a while until you understand what the impact is.  Only cut over to other actions when you are comfortable with the results.
